...
class Intelligence():
    def __init__(self,knowledge, uname='buddy'):
    ...
    def reply(self, intent, query):
        if intent in self.response_dict.keys():
            if intent == 'asked ques':              
                result = requests.get(BIDIRECTIONAL_API_LINK, params={'paragraph': self._knowledge, 'question':query}, proxies=DEVELOPMENT_PROXY)                   
                try:                    
                    json_object = json.loads(result.text)                    
                    self.current_intent = 'confirm'
                    answer = json_object['result']
                    if not answer:
                        self.current_intent='undefined ques'                                
